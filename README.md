...
### 4️⃣ 옵시디언 스타일 커스터마이징 — 코어 단순 + 옵트인 풍부함

Pepper 는 **하나의 frame을 강요하지 않습니다**. 메인 워크플로우(변경사항 → 커밋, 커밋 로그)는 표준 Git GUI 패턴 그대로 가벼움. 풍부함은 사용자가 *원하는 만큼만* 켜서 자기 색깔로 조립합니다.

| 옵트인 요소 | 라이트 유저 | 파워 유저 |
|---|---|---|
| 테마 | Catppuccin Mocha 기본 | 4 flavor + Custom 팔레트 + **AI 자연어 생성** + 수동 편집 |
| 배경 데코 | OFF 기본 | 9 옵션 + 36 아이콘 + AI 자연어 ("고양이가 떠다니는") |
| 페퍼 배지 | 기본값 임계값 | 핫 / stale 임계값 슬라이더 (30~730일) |
| AI provider | 사용 안 함 | 로컬 llama.cpp (BYOK 없이) / Anthropic BYOK 둘 다 선택 |
| 변경사항 되돌리기 | 우클릭 단순 | 다중선택 / 전체 / untracked 옵트인 등 깊이 있음 |
| 사이드바 / 행 여백 / 날짜 포맷 / `--all` 토글 | 기본 그대로 | 모든 표시 옵션 개인화 |

**진행형 정체성** — 다른 Git GUI들이 "완성된 도구" 라면 Pepper 는 *내가 키워가는 도구*. 다음 발전 방향:

- 핫키 커스터마이징 (옵시디언 `hotkeys.json` 패턴)
- 워크스페이스 레이아웃 저장 / 복원
- 사용자 스니펫 (커밋 메시지 템플릿 / 정규식)
- 플러그인 시스템 (코어 안정 후)
...
- **Frame insight**: "본인용 vs 시장 출시" 이분법이 잘못된 frame이었음 — 실제 시나리오는 그 중간(소규모 신뢰권)이거나 **진행형 프로젝트의 첫 안정판**
